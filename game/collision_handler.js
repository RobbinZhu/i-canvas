...
export default function(collideLayer) {
    if (!collideLayer) {
        return
    }
    const myWidth = this.width
    const myHeight = this.height
    const ax = myWidth * this.anchor.x
    const ay = myHeight * this.anchor.y
    ...
    let resolveY = resolveYCollision(vy, items, vyAbs, x1, y1, x2, y2, this)

    const velocityCorrection = this.velocityCorrection
    if (resolveX) {
        this.velocity.x = 0
        velocityCorrection.x = resolveX
    }
    ...
    if (resolveY) {
        if (!this.touchWater) {
            ...
            this.velocity.y += resolveY
            this.velocity.y = Math.round(this.velocity.y)
            vy = this.velocity.y
            resolveY = 0
        }
        velocityCorrection.y = resolveY
    }

    velocityCorrection.x += vx
    velocityCorrection.y += vy
}